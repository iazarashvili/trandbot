"""6-month backtest, 3 symbols, $3000 shared balance, monthly stats."""
import sys
import importlib
...
symbols_cfg = [
    ("BTCUSD", 0.03, "config_btcusd"),
    ("XAUUSD", 0.03, "config_xauusd"),
...
for sym, lot, cfg_name in symbols_cfg:
    ...
    use_liq = getattr(cfg, "USE_LIQUIDITY_TP", True)
    use_part = getattr(cfg, "USE_PARTIAL_CLOSE", True)
    rrr = getattr(cfg, "RRR", 3.0)
    be_r = getattr(cfg, "BREAKEVEN_R", 0.0) if getattr(cfg, "USE_BREAKEVEN", False) else 0.0

    res = eng.run(d["frames"]["m5"], d["frames"]["h1"], d["frames"]["m15"],
                  d["contract"], d["point"], rrr=rrr, invert=False,
                  use_liq_tp=use_liq, use_partial=use_part, breakeven_r=be_r)
...
print("=" * 110)
print(f"  PORTFOLIO BACKTEST | {START.strftime('%Y-%m-%d')} to {END.strftime('%Y-%m-%d')} | Balance: ${BALANCE:,.0f}")
print(f"  BTCUSD: 0.03 lot | XAUUSD: 0.03 lot (sweep+BE) | GBPUSD: 0.05 lot")
print("=" * 110)
print()

# Monthly table with per-symbol breakdown
header = f"{'Month':<10}"
for s in syms:
    header += f" | {s:>20}"
...
# Per symbol summary
print()
print(f"{'Symbol':<10} {'Lot':>5} {'Trades':>7} {'Wins':>5} {'Loss':>6} {'WR%':>7} {'Net P&L':>12} {'PF':>6}")
print("-" * 65)
...
for sym in syms:
    ...
    lot = [l for s, l, c in symbols_cfg if s == sym][0]
    wins = len([t for t in tt if t["pnl"] > 0])
    losses = len([t for t in tt if t["pnl"] <= 0])
    net = sum(t["pnl"] for t in tt)
    gw = sum(t["pnl"] for t in tt if t["pnl"] > 0)
    gl = -sum(t["pnl"] for t in tt if t["pnl"] <= 0)
    pf = round(gw / gl, 2) if gl > 0 else "-"
    wr = round(wins / len(tt) * 100, 1) if tt else 0
    sign = "+" if net >= 0 else ""
    print(f"{sym:<10} {lot:>5} {len(tt):>7} {wins:>5} {losses:>6} {wr:>6.1f}% {sign}${net:>10,.2f} {str(pf):>6}")
    total_trades += len(tt)
    total_wins += wins
    total_losses += losses
    total_pnl += net
...
print(f"{'TOTAL':<10} {'':>5} {total_trades:>7} {total_wins:>5} {total_losses:>6} {wr:>6.1f}% {sign}${total_pnl:>10,.2f} {str(pf):>6}")
